...
@app.get("/other_page")
def other_page(request: Request, noisy_image: str = None, histogram: str = None):
    return templates.TemplateResponse("other_page.html", {"request": request, "noisy_image": noisy_image, "histogram": histogram})
...
async def add_noise(request: Request, file: UploadFile = File(...), noise_level: float = Form(0.5), resp: str = Form(...)):
    try:
        ...
        if result["success"]:
            ...
            # Перенаправляем на другую вкладку с обработанными изображениями
            return templates.TemplateResponse("other_page.html", {"request": request, "noisy_image": noisy_image_base64, "histogram": histogram_base64})
        else:
